...
test("pages and filters capability summaries without emitting schemas by default", async () => {
    ...
    assert.equal(defaultPayload.total, catalog.capabilities.filter((capability) => capability.lifecycle.canonicalId === capability.id).length);
...
test("keeps ue_context directory-only by default and pages full schemas by domain", () => {
    ...
    assert.equal(domainPayload.total, catalog.forDomain("blueprint").filter((capability) => capability.lifecycle.canonicalId === capability.id).length);
...
test("registers one MCP caller session and attributes subsequent requests", async () => {
    ...
    assert.equal(calls[0]?.url.endsWith("/api/v1/clients/register"), true);
    ...
    assert.equal(calls[2]?.url.endsWith("/api/v1/clients/unregister"), true);
...
test("uses the v1 client registration body and falls back to Legacy HTTP", async () => {
    ...
    const registrations = calls.filter((call) => call.url.endsWith("/api/v1/clients/register"));
    assert.equal(registrations.length, 2);
    const registrationBody = JSON.parse(registrations[0]?.body ?? "{}");
    assert.deepEqual(Object.keys(registrationBody).sort(), [
        "clientKind",
        "instanceId",
        "name",
        "pid",
    ...
    assert.equal(calls.some((call) => call.url.endsWith("/api/v1/clients/unregister")), false);
});
test("re-registers after a heartbeat rejects an expired session", async () => {
    ...
    const fetchImpl = async (input, init) => {
        ...
        if (url.endsWith("/api/v1/clients/heartbeat") &&
            !heartbeatRejected) {
    ...
    const healthCall = calls.find((call) => call.url.endsWith("/api/health"));
    assert.equal(healthCall?.headers.get("x-ueai-session-id"), "client-session-2");
});
