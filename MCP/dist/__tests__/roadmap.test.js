...
function ue53PackageWithRegistryTag() {
    const chunks = [];
...
test("runs UE package header reads in the isolated Asset Worker", async () => {
    ...
    const result = await new LocalAssetExecutor().execute("production.asset.package.summary.get", { projectRoot: root, assetPath: "Header.uasset", engineVersion: "5.3" });
    assert.equal(result.packageSummary.tag, "0x9e2a83c1");
    assert.equal(result.assetRegistry.objectCount, 1);
    assert.equal(((result.assetRegistry.assets[0]?.tags) ?? {}).SourceFile, "x.png");
});
test("propagates AbortSignal and posts best-effort Editor cancellation", async () => {
    ...
    controller.abort();
    await assert.rejects(pending, (error) => error instanceof UEApiError && error.code === "request_cancelled");
