...
function ue53PackageWithRegistryTag(tagCount = 1, tagValue = "x.png") {
    const chunks = [];
    const i32 = (value) => { const bytes = Buffer.alloc(4); bytes.writeInt32LE(value); chunks.push(bytes); };
    const u32 = (value) => { const bytes = Buffer.alloc(4); bytes.writeUInt32LE(value >>> 0); chunks.push(bytes); };
    const u16 = (value) => { const bytes = Buffer.alloc(2); bytes.writeUInt16LE(value); chunks.push(bytes); };
    const i64 = (value) => { const bytes = Buffer.alloc(8); bytes.writeBigInt64LE(BigInt(value)); chunks.push(bytes); };
    const string = (value) => { const content = Buffer.from(`${value}\0`, "latin1"); i32(content.length); chunks.push(content); };
    ...
    i32(1);
    string("Header");
    string("/Script/Engine.Texture2D");
    i32(tagCount);
    for (let index = 0; index < tagCount; index += 1) {
        string(index === 0 ? "SourceFile" : `Tag${index}`);
        string(tagValue);
    }
...
test("validates bounded Recipe retries and restricted SAL plan-only source", () => {
    ...
    assert.throws(() => parseSalSource("import fs from 'node:fs';"), (error) => error instanceof UEApiError && error.code === "sal_ast_forbidden");
...
test("migrates v1 Recipe journals and rejects duplicate recovery of an active Worker", () => {
    ...
    assert.throws(() => runner.resume(runId, base.planDigest), (error) => error instanceof UEApiError && error.code === "recipe_run_active");
});
test("resumes an interrupted Recipe from the next checkpoint without replaying completed steps", async () => {
    ...
    try {
        ...
        assert.deepEqual(dispatched.map((item) => item.requestId), [`${started.runId}-first`, `${started.runId}-second`]);
    }
    finally {
...
test("runs UE package header reads in the isolated Asset Worker", async () => {
    ...
    const result = await new LocalAssetExecutor().execute("production.asset.package.summary.get", { projectRoot: root, assetPath: "Header.uasset", engineVersion: "5.3", detail: "full", sections: ["assetRegistry"] });
    assert.equal(result.packageSummary.tag, "0x9e2a83c1");
    const registry = result.sections.assetRegistry;
    assert.equal(registry.objectCount, 1);
    assert.equal(registry.assets[0]?.tags.find((tag) => tag.key === "SourceFile")?.value, "x.png");
});
test("keeps default Asset summaries inside the token budget and full tables valid", async () => {
    ...
    const compact = await executor.execute("production.asset.package.summary.get", { projectRoot: root, assetPath: "LargeHeader.uasset", engineVersion: "5.3" });
    ...
    const full = await executor.execute("production.asset.package.summary.get", {
        projectRoot: root,
    ...
    assert.equal(typeof full.budget.estimatedBytes, "number");
    const paged = await executor.execute("production.asset.package.summary.get", {
        projectRoot: root,
        assetPath: "LargeHeader.uasset",
        engineVersion: "5.3",
        detail: "full",
    ...
    const compactDiff = await executor.execute("production.asset.package.diff", {
        projectRoot: root,
        assetPath: "LargeHeader.uasset",
...
test("resolves project, additional, and Engine plugin dependencies offline", async () => {
    ...
    const result = await new LocalProjectExecutor().execute("production.project.validate", { projectRoot: root, engineRoot: engine });
    ...
    const withoutEngine = await new LocalProjectExecutor().execute("production.project.validate", { projectRoot: root });
    assert.equal(withoutEngine.valid, true);
    assert.ok(withoutEngine.diagnostics.some((diagnostic) => diagnostic.code === "plugin_unresolved_external" && diagnostic.severity === "info"));
    const missingRequired = await new LocalProjectExecutor().execute("production.project.validate", { projectRoot: root, engineRoot: engine });
    assert.equal(missingRequired.valid, false);
    assert.ok(missingRequired.diagnostics.some((diagnostic) => diagnostic.code === "plugin_dependency_missing" && diagnostic.severity === "error"));
});
test("propagates AbortSignal and posts best-effort Editor cancellation", async () => {
    ...
    controller.abort();
    await assert.rejects(pending, (error) => error instanceof UEApiError
...
test("discovers opted-in Development targets and requires the exact attach digest", async () => {
    ...
    await assert.rejects(executor.execute("production.development.attach", { projectRoot: root, pid: process.pid, scope: "observe", approvePlanDigest: `sha256:${"4".repeat(64)}`, confirmAttach: true }), (error) => error instanceof UEApiError && error.code === "attach_approval_required");
});
