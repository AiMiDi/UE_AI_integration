...
import { chmodSync, copyFileSync, linkSync, mkdirSync, mkdtempSync, readFileSync, realpathSync, rmSync, writeFileSync, } from "node:fs";
...
import { deriveTraceWorkerEndpoint, locateTraceWorker, TraceWorkerClient, } from "../trace-worker.js";
import { UEApiError } from "../ue-bridge.js";
function serviceEndpoint(directory) {
...
function fakeWorkerBundle(directory) {
    ...
    writeFileSync(join(engineDirectory, "Build", "Build.version"), JSON.stringify({ MajorVersion: 5, MinorVersion: 3, PatchVersion: 0 }), "utf8");
    const insightsPath = process.platform === "darwin"
        ? join(engineDirectory, "Binaries", "Mac", "UnrealInsights.app", "Contents", "MacOS", "UnrealInsights")
        : join(engineDirectory, "Binaries", process.platform === "win32" ? "Win64" : "Linux", process.platform === "win32" ? "UnrealInsights.exe" : "UnrealInsights");
    ...
    return {
        ...
        env,
        resource: (relativePath) => join(root, relativePath),
        refreshDigests,
    };
}
...
test("locates packaged and source-built Trace Workers deterministically", () => {
    ...
    const moduleUrl = pathToFileURL(join(directory, "MCP", "dist", "trace-worker.js")).href;
    ...
    try {
        ...
        const source = join(directory, "Programs", "UEAITraceWorker", "Binaries", platform, name);
        mkdirSync(dirname(source), { recursive: true });
        writeFileSync(source, "fixture", "utf8");
        const staged = join(directory, "Intermediate", "TraceWorkerStage", "Tools", "Trace", platform, "5.3", name);
...
test("binds service endpoints to the canonical Engine directory", () => {
    ...
    try {
        ...
        const endpointAEquivalent = deriveTraceWorkerEndpoint(worker, process.env, "5.3", resolve(first, "."));
...
test("reuses the user-scoped Trace Worker service and keeps stdio diagnostic fallback", async () => {
    ...
    try {
        ...
        assert.equal(resolve(String(handshake.unrealInsightsEngineDir)), realpathSync.native(String(bundle.env.UEAI_ENGINE_ROOT) + sep + "Engine"));
        const serverPid = handshake.serverPid;
        const result = await client.execute("production.trace.provider.list", { tracePath: "sample.utrace", backend: "local" }, "request-1");
        ...
        assert.equal((await stdio.handshake()).protocolVersion, 1);
    }
    finally {
...
test("fails closed when UEAI_TRACE_WORKER has no Worker-bundled Resources", async () => {
    ...
    try {
        ...
        await assert.rejects(client.handshake(true), (error) => error instanceof UEApiError &&
...
test("rejects a valid but divergent Worker resource bundle", async () => {
    ...
    const bundle = fakeWorkerBundle(directory);
    writeFileSync(script, workerScript, "utf8");
    rewriteJson(bundle.resource("Resources/Capabilities/production.json"), (production) => {
    ...
    try {
        const client = new TraceWorkerClient({
            ...
            transport: "stdio",
            timeoutMs: 5_000,
            moduleUrl: pathToFileURL(join(repositoryRoot, "MCP", "dist", "trace-worker.js")).href,
            env: bundle.env,
        });
        await assert.rejects(client.handshake(true), (error) => error instanceof UEApiError &&
            error.code === "trace_worker_contract_mismatch" &&
...
test("validates every trusted Trace contract resource before accepting its digest", async (t) => {
    const cases = [
        ...
        },
        {
            ...
            relativePath: "Resources/Trace/worker-protocol.v1.json",
            mutate: (value) => {
                const resident = value.residentService;
                resident.tcp = true;
        ...
        },
        {
            ...
            mutate: (value) => {
                ...
                value.capabilities = capabilities.filter((capability) => capability.id !== "production.trace.timing.query");
            },
        },
        {
        ...
        },
        {
            ...
            mutate: (value) => {
                ...
                value.panels = panels.filter((panel) => panel.capability !== "production.trace.screenshot.query");
            },
        },
        {
            ...
            mutate: (value) => {
                const panels = value.panels;
                const timing = panels.find((panel) => panel.capability === "production.trace.timing.query");
                const operations = timing?.operations;
                timing.operations = operations.filter((operation) => operation !== "cpuSampling");
            },
        },
        {
...
test("restricts MCP imports to Saved, Worker store, and declared roots", async () => {
    ...
    const bundle = fakeWorkerBundle(directory);
    for (const path of [saved, store, declared, outside])
    ...
    try {
        ...
        await assert.rejects(client.execute("production.trace.import", { tracePath: files[3] }), (error) => error instanceof UEApiError && error.code === "trace_path_not_allowed");
    }
    finally {
        await allowFakeServiceToExit();
        removeTemporaryDirectory(directory);
...
test("routes declared localTrace capabilities and honors explicit backend selection", async () => {
    ...
    const calls = [];
    const router = new BackendRoutingExecutor(catalog, {
        execute: async () => {
            calls.push("editor");
    ...
    assert.deepEqual(await router.execute(descriptor.id, { backend: "editor" }), { backend: "editor" });
...
test("auto falls back only when the local worker is unavailable", async () => {
    ...
    await assert.rejects(router.execute(descriptor.id, { backend: "local" }), (error) => error instanceof UEApiError &&
...
test("editor-preferred auto falls back only when the Editor is unreachable", async () => {
    ...
    await assert.rejects(router.execute(descriptor.id, { backend: "editor" }), (error) => error instanceof UEApiError && error.code === "editor_unreachable");
    assert.equal(localCalls, 1);
    editorError = "invalid_request";
    await assert.rejects(router.execute(descriptor.id, { backend: "auto" }), (error) => error instanceof UEApiError && error.code === "invalid_request");
    assert.equal(localCalls, 1);
});
test("binds trace starts and durable IDs to their owning backend", async () => {
    ...
    await assert.rejects(router.execute(start.id, {
        backend: "editor",
        target: { kind: "development", launchProfileId: "dev" },
    }), (error) => error instanceof UEApiError &&
...
test("ue_production executes a local trace query while Editor is offline", async () => {
    ...
    const registered = runtime.server._registeredTools;
...
//# sourceMappingURL=trace-worker.test.js.map