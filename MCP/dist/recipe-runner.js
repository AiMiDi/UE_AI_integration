...
import { loadCapabilityCatalog, capabilityIsReadOnly, } from "./capability-catalog.js";
...
function recipeRoot(env = process.env) {
    if (env.UEAI_RECIPE_ROOT)
        return resolve(env.UEAI_RECIPE_ROOT);
...
function descriptorSessionSafe(descriptor) {
    return descriptor.traits.sessionSafe === true
        && descriptor.effects.asset !== "write"
        && descriptor.effects.external !== "write";
}
export function recipeHasSessionSteps(value) {
    return isObject(value)
        && Array.isArray(value.steps)
        && value.steps.some((step) => isObject(step) && step.kind === "sessionCapability");
}
function capabilityCatalogDigest(catalog) {
    return digest(["blueprint", "scene", "content", "animation", "ai", "production"]
        .flatMap((domain) => catalog.forDomain(domain))
        .sort((left, right) => left.id.localeCompare(right.id)));
}
function pieState(value) {
    if (!isObject(value))
        return "unknown";
...
function pieIsStopped(value) {
    return ["stopped", "idle", "none", "inactive"].includes(pieState(value).toLowerCase());
}
function assertSessionReferences(value, binding, path = "params") {
    if (Array.isArray(value)) {
...
function defaultRetry(value) {
    return {
        ...
        transientErrors: Array.isArray(value?.transientErrors)
            ? value.transientErrors.filter((item) => typeof item === "string")
            : [
                "editor_unreachable",
...
function validationError(diagnostics, path, code, message) {
    diagnostics.push({ path, code, message });
}
function validateCondition(value, path, diagnostics, allowPollOutput = false) {
    if (!isObject(value)) {
...
export function validateRecipe(value, catalog = loadCapabilityCatalog()) {
    const diagnostics = [];
    if (!isObject(value)) {
    ...
    for (const field of Object.keys(value)) {
        if (!allowedRoot.has(field))
            validationError(diagnostics, `/${field}`, "recipe_field_forbidden", `Unknown Recipe field ${field}.`);
    }
    if (value.schema !== RECIPE_SCHEMA)
        validationError(diagnostics, "/schema", "recipe_schema_invalid", `schema must be ${RECIPE_SCHEMA}.`);
    if (typeof value.id !== "string" || !/^[a-z0-9][a-z0-9.-]{0,127}$/.test(value.id))
        validationError(diagnostics, "/id", "recipe_id_invalid", "id must be a stable lower-case identifier.");
    if (typeof value.version !== "string" || value.version.length === 0)
        validationError(diagnostics, "/version", "recipe_version_invalid", "version is required.");
    ...
    if (Array.isArray(value.steps)) {
        value.steps.forEach((raw, index) => {
            ...
            if (typeof raw.id !== "string" || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/.test(raw.id))
                validationError(diagnostics, `${path}/id`, "recipe_step_id_invalid", "Step id is invalid.");
            else if (ids.has(raw.id))
                validationError(diagnostics, `${path}/id`, "recipe_step_id_duplicate", "Step id is duplicated.");
            else
                ids.add(raw.id);
            ...
            if (["sessionCapability", "workflow", "approval"].includes(String(raw.kind)) || raw.highRisk === true)
            ...
            if (raw.kind === "capability") {
                ...
                if (!descriptor)
                    validationError(diagnostics, `${path}/capability`, "capability_not_found", "Capability is not present in the local catalog.");
                else if (!descriptorReadOnly(descriptor))
                    validationError(diagnostics, `${path}/capability`, "recipe_direct_write_forbidden", "Direct Recipe capability steps must be asset/world read-only; use Workflow for writes.");
            }
            if (raw.kind === "sessionCapability") {
                const descriptor = typeof raw.capability === "string" ? catalog.get(raw.capability) : undefined;
                if (!descriptor)
                    validationError(diagnostics, `${path}/capability`, "capability_not_found", "Capability is not present in the local catalog.");
                else if (!descriptorSessionSafe(descriptor))
                    validationError(diagnostics, `${path}/capability`, "recipe_session_capability_forbidden", "Session Recipe steps require traits.sessionSafe=true and cannot write assets or external state.");
            }
            ...
            if (raw.kind === "workflow") {
                if (!isObject(raw.workflow))
                    validationError(diagnostics, `${path}/workflow`, "workflow_required", "Workflow steps require an inline Workflow v2 object.");
                else if (raw.workflow.dsl !== "ue.workflow" || raw.workflow.dslVersion !== "2.0")
                    validationError(diagnostics, `${path}/workflow`, "workflow_v2_required", "Recipe writes accept only dsl=ue.workflow and dslVersion=2.0.");
            }
            if (raw.kind === "poll") {
                if (!isObject(raw.poll))
                    validationError(diagnostics, `${path}/poll`, "poll_invalid", "poll configuration is required.");
                else {
                    const descriptor = typeof raw.poll.capability === "string" ? catalog.get(raw.poll.capability) : undefined;
                    if (!descriptor || !descriptorReadOnly(descriptor))
                        validationError(diagnostics, `${path}/poll/capability`, "poll_capability_forbidden", "Poll requires a read-only capability.");
                    const attempts = Number(raw.poll.maxAttempts);
                    const interval = Number(raw.poll.intervalMs);
                    if (!Number.isInteger(attempts) || attempts < 1 || attempts > MAX_POLL_ATTEMPTS)
                        validationError(diagnostics, `${path}/poll/maxAttempts`, "poll_unbounded", `Poll maxAttempts must be 1-${MAX_POLL_ATTEMPTS}.`);
                    if (!Number.isInteger(interval) || interval < 100 || interval > MAX_POLL_INTERVAL_MS)
                        validationError(diagnostics, `${path}/poll/intervalMs`, "poll_interval_invalid", `Poll intervalMs must be 100-${MAX_POLL_INTERVAL_MS}.`);
                    validateCondition(raw.poll.until, `${path}/poll/until`, diagnostics, true);
                }
            }
            if (raw.kind === "condition")
                validateCondition(raw.condition, `${path}/condition`, diagnostics);
            if (raw.compensate !== undefined) {
                if (!isObject(raw.compensate) || !["capability", "sessionCapability", "workflow"].includes(String(raw.compensate.kind)))
                    validationError(diagnostics, `${path}/compensate`, "compensation_invalid", "Compensation must be a capability, sessionCapability, or Workflow action.");
                if (isObject(raw.compensate) && raw.compensate.kind === "capability") {
                    ...
                    if (!descriptor || !descriptorReadOnly(descriptor))
                        validationError(diagnostics, `${path}/compensate/capability`, "compensation_write_forbidden", "Write compensation must use Workflow rollback, not a direct capability.");
                }
                if (isObject(raw.compensate) && raw.compensate.kind === "sessionCapability") {
                    const descriptor = typeof raw.compensate.capability === "string" ? catalog.get(raw.compensate.capability) : undefined;
                    if (!descriptor || !descriptorSessionSafe(descriptor))
                        validationError(diagnostics, `${path}/compensate/capability`, "compensation_session_capability_forbidden", "Session compensation requires a sessionSafe capability.");
                }
                if (isObject(raw.compensate) &&
                    raw.compensate.kind === "workflow" &&
                    (typeof raw.compensate.runId !== "string" ||
                        !/^\$(inputs|steps)\.[A-Za-z0-9_.-]+$/.test(raw.compensate.runId))) {
                    validationError(diagnostics, `${path}/compensate/runId`, "workflow_compensation_run_id_required", "Workflow compensation must reference a persisted Workflow runId from inputs or a prior step output.");
                }
            }
            for (const forbidden of ["script", "command", "shell", "python", "javascript", "loop", "while"]) {
                if (forbidden in raw)
                    validationError(diagnostics, `${path}/${forbidden}`, "recipe_script_forbidden", "Arbitrary scripts and unbounded loops are forbidden.");
            }
        });
        ...
        if (firstWorkflow >= 0 && (firstCheckout < 0 || firstCheckout > firstWorkflow)) {
            validationError(diagnostics, `/steps/${firstWorkflow}`, "source_control_checkout_required", "Every controlled Workflow write requires a preceding sourceControlCheckout step so checkout failure remains zero-write.");
        }
    }
    if (diagnostics.length > 0)
        return { ok: false, schema: "ue.recipe-validation.v2", diagnostics };
...
function evaluate(condition, context) {
    ...
    if (Array.isArray(condition.all))
        return condition.all.every((item) => evaluate(item, context));
    if (Array.isArray(condition.any))
        return condition.any.some((item) => evaluate(item, context));
    if (condition.not !== undefined)
        return !evaluate(condition.not, context);
    for (const operator of ["equals", "notEquals", "exists"]) {
...
function materialize(value, context) {
    ...
    if (typeof value === "string" && /^\$(inputs|steps)\.[A-Za-z0-9_.-]+$/.test(value))
        return lookup(context, value.slice(1));
    return value;
}
function materializeInputs(value, inputs) {
    if (Array.isArray(value))
        return value.map((item) => materializeInputs(item, inputs));
    if (isObject(value)) {
        return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, materializeInputs(item, inputs)]));
    }
...
export class RecipeRunner {
    ...
    plan(recipe, inputs = {}) {
        ...
        if (!validation.ok)
            throw new UEApiError({ code: "recipe_invalid", message: "Recipe validation failed.", details: validation.diagnostics });
        return this.planResult(validation);
    }
    async planOnline(recipe, inputs = {}) {
        const validation = this.prepare(recipe, inputs);
        if (!validation.ok || !validation.normalized)
            throw new UEApiError({ code: "recipe_invalid", message: "Recipe validation failed.", details: validation.diagnostics });
        if (!recipeHasSessionSteps(validation.normalized))
            return this.planResult(validation);
    ...
    startPrepared(validation, inputs, approvePlanDigest, session) {
        if (!validation.ok || !validation.normalized || !validation.planDigest)
            throw new UEApiError({ code: "recipe_invalid", message: "Recipe validation failed.", details: validation.diagnostics });
    ...
    start(recipe, inputs = {}, approvePlanDigest) {
        if (recipeHasSessionSteps(recipe)) {
    ...
    async startOnline(recipe, inputs = {}, approvePlanDigest) {
    ...
    result(runId) {
        const state = readRun(runId, this.env);
        if (!["completed", "failed", "cancelled"].includes(state.status))
            throw new UEApiError({ code: "recipe_result_not_ready", message: "Recipe result is available only after a terminal state.", details: { status: state.status, phase: state.phase } });
        return state;
    }
    list(limit = 50) {
        ...
        return readdirSync(root, { withFileTypes: true })
            ...
            .flatMap((entry) => {
            try {
                return [readRun(entry.name, this.env)];
            }
            catch {
                return [];
            }
...
async function invokeStep(client, step, context, state) {
    if (step.kind === "condition") {
        if (!evaluate(step.condition, context))
            throw new UEApiError({ code: "recipe_condition_failed", message: `Condition step ${step.id} evaluated to false.` });
        return { matched: true };
    }
    ...
    if (step.kind === "workflow") {
        ...
        if (typeof approvedDigest !== "string")
            throw new UEApiError({ code: "approval_required", message: `Workflow step ${step.id} has no approved planDigest.` });
        const request = {
            action: "execute",
            workflow: materialize(step.workflow, context),
    ...
    if (step.kind === "capability" || step.kind === "sessionCapability") {
        return client.execute(step.capability, materialize(step.params ?? {}, context), `${state.runId}-${step.id}`);
    }
    ...
    for (let attempt = 1; attempt <= poll.maxAttempts; attempt += 1) {
        ...
        if (latest.cancelRequested)
            throw new UEApiError({ code: "recipe_cancelled", message: "Recipe cancellation reached a poll boundary." });
        output = await client.execute(poll.capability, materialize(poll.params ?? {}, context), `${state.runId}-${step.id}-poll-${attempt}`);
        const pollContext = { ...context, poll: { output } };
        if (evaluate(poll.until, pollContext))
...
async function compensate(client, state, completed, context) {
    for (const step of completed.reverse()) {
        if (!step.compensate)
            continue;
        try {
            const compensationParams = materialize(step.compensate.params ?? {}, context);
...
async function assertSessionGeneration(client, state) {
    if (!state.session || state.session.activePieGeneration === undefined)
        return;
    const health = await client.getHealth();
    ...
    const current = await client.execute("scene.pie.status", {}, `${state.runId}-pie-generation-check-${state.nextStep}`);
...
async function prepareSessionExecution(client, state) {
    if (!state.session)
        return;
    const health = await client.getHealth();
    if (String(health.serverInstanceId ?? "") !== state.session.serverInstanceId) {
    ...
    const current = await client.execute("scene.pie.status", {}, `${state.runId}-pie-preflight`);
    ...
    const lease = await client.execute("production.lease.acquire", { type: "pie" }, `${state.runId}-pie-lease`);
    ...
    try {
        if (state.session.activePieGeneration === undefined) {
            ...
            while (Date.now() < deadline) {
                const candidate = await client.execute("scene.pie.status", {}, `${state.runId}-pie-start-status-${Date.now()}`);
        ...
        try {
            await client.execute("production.lease.release", { type: "pie" }, `${state.runId}-pie-start-failure-lease-release`);
        }
        catch {
...
async function cleanupSessionExecution(client, state) {
    if (!state.session)
        return;
    ...
    try {
        await client.execute("production.lease.release", { type: "pie" }, `${state.runId}-pie-lease-release`);
...
export async function executeRecipeRun(runId, endpoint, workerInstanceId) {
    ...
    try {
        ...
        for (; state.nextStep < state.recipe.steps.length;) {
            ...
            if (!succeeded)
                throw new UEApiError({ code: "recipe_step_failed", message: `Step ${step.id} failed.` });
        ...
        if (sessionPrepared)
            await cleanupSessionExecution(client, state);
        try {
            const latest = readRun(runId);
