...
export function validateRecipe(value, catalog = loadCapabilityCatalog()) {
    ...
    if (Array.isArray(value.steps)) {
        value.steps.forEach((raw, index) => {
            ...
            if (["workflow", "approval"].includes(String(raw.kind)) || raw.highRisk === true)
            ...
            if (raw.compensate !== undefined) {
                if (!isObject(raw.compensate) || !["capability", "workflow"].includes(String(raw.compensate.kind)))
                    validationError(diagnostics, `${path}/compensate`, "compensation_invalid", "Compensation must be a capability or Workflow action.");
    ...
    for (const step of completed.reverse()) {
        ...
        try {
            const output = step.compensate.kind === "workflow"
                ...
                : await client.execute(step.compensate.capability, materialize(step.compensate.params ?? {}, context), `${state.runId}-${step.id}-compensate`);
