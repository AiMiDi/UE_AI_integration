...
function tableDescriptor(bytes, summary, count, offset) {
    if (count === 0 || offset === 0)
        return { count, offset, available: count === 0 };
...
function parseNames(bytes, summary) {
    if (summary.nameCount === 0)
        return { count: 0, offset: summary.nameOffset, names: [] };
...
function parseDependencies(bytes, summary, names) {
    const result = { dependsMap: { exportCount: summary.exportCount, offset: summary.dependsOffset, edges: 0 } };
...
function parseAssetRegistry(bytes, summary) {
    if (summary.assetRegistryDataOffset <= 0)
        return { available: false, reason: "asset_registry_section_absent", assets: [] };
    const reader = new PackageReader(bytes);
    ...
    for (let objectIndex = 0; objectIndex < objectCount; objectIndex += 1) {
        ...
        if (assets.length < MAX_PREVIEW_ASSETS)
            assets.push({ objectPath, objectClass, tagCount, tags, tagMetadata, tagsTruncated: retained < tagCount });
    }
    return {
        available: true,
...
function summary(path, requestedEngineVersion) {
