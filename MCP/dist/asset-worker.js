...
class PackageReader {
    ...
    i64(label) {
        ...
        if (!Number.isSafeInteger(numeric))
            throw failure("asset_corrupt", `${label} is outside the safe offset range.`);
        return numeric;
    }
    guid(label) {
    ...
    string(label) {
        ...
        if (units > MAX_STRING_UNITS)
            throw failure("asset_corrupt", `${label} exceeds the string limit.`);
    ...
    count(label, maximum = MAX_ARRAY_ITEMS) {
        const value = this.i32(label);
        if (value < 0 || value > maximum)
            throw failure("asset_corrupt", `${label} is outside the supported range.`);
        return value;
    }
}
...
function outputOptions(request) {
    ...
    const allowed = ["names", "imports", "exports", "assetRegistry", "dependencies"];
...
function pageMetadata(total, offset, returned, limit) {
...
function readPrefix(path, maximum) {
...
function headerPath(requestedPath) {
    if (extname(requestedPath).toLowerCase() !== ".uexp")
        return { headerPath: requestedPath };
...
function engineVersion(reader, label) {
    return {
...
function parseSummary(reader) {
    ...
    if (tag !== PACKAGE_TAG)
        throw failure("asset_corrupt", "Asset package tag is invalid, encrypted, or unsupported.");
    ...
    for (let index = 0; index < additionalPackageCount; index += 1)
        reader.string(`additionalPackagesToCook[${index}]`);
...
function tableDescriptor(bytes, summary, count, offset, options) {
    ...
    if (offset < 0 || offset >= summary.totalHeaderSize)
        throw failure("asset_corrupt", "Package table offset is outside TotalHeaderSize.");
    const endOffset = nextOffset(summary, offset);
    if (endOffset < offset || endOffset > bytes.length)
        throw failure("asset_corrupt", "Package table range is invalid.");
...
function parseNames(bytes, summary, options) {
    if (summary.nameCount === 0)
        return { offsetBytes: summary.nameOffset, items: [], ...pageMetadata(0, options.offset, 0, options.limit) };
    ...
    if (reader.position > nextOffset(summary, summary.nameOffset))
        throw failure("asset_corrupt", "Name table overlaps the next package section.");
    return {
...
function nameAt(names, index, number) {
    if (index < 0 || index >= names.length)
        return null;
    return number > 0 ? `${names[index]}_${number - 1}` : names[index];
}
function parseNameList(bytes, summary) {
    if (summary.nameCount === 0)
        return [];
    const reader = new PackageReader(bytes);
    reader.seek(summary.nameOffset, "name table");
...
function parseDependencies(bytes, summary, names, options) {
    const result = {
        dependsMap: {
    ...
    if (summary.softPackageReferencesCount > 0 && summary.softPackageReferencesOffset > 0) {
        ...
        for (let index = 0; index < summary.softPackageReferencesCount; index += 1) {
            ...
            if (index >= options.offset && index < options.offset + options.limit)
                softPackageReferences.push(value);
        }
    }
    ...
    const depends = result.dependsMap;
    const soft = result.softPackageReferences;
...
function parseAssetRegistry(bytes, summary, options) {
...
function compactPackageSummary(value) {
    const result = { ...value };
    const chunkIds = Array.isArray(result.chunkIds) ? result.chunkIds : [];
    result.chunkIds = {
...
function updatePageAfterTrim(section, itemField) {
    const items = Array.isArray(section[itemField]) ? section[itemField] : [];
...
function updateDependencyPageAfterTrim(section) {
    const depends = section.dependsMap;
    const soft = section.softPackageReferences;
    const total = Math.max(Number(depends?.total ?? 0), Number(soft?.total ?? 0));
    const returned = Math.max(Number(depends?.returned ?? 0), Number(soft?.returned ?? 0));
...
function enforceBudget(value, options) {
    ...
    const trimOne = () => {
        ...
        const candidates = [
            [sections?.dependencies, "perExportCounts"],
            [sections?.dependencies, "softPackageReferences"],
            [assetRegistry, "assets"],
            [sections?.exports, "items"],
            [sections?.imports, "items"],
            [sections?.names, "items"],
        ];
        for (const [section, field] of candidates) {
            ...
            if (field === "perExportCounts") {
                const depends = section.dependsMap;
                const items = Array.isArray(depends?.perExportCounts) ? depends.perExportCounts : [];
                if (items.length > 0) {
                    items.pop();
            ...
            if (field === "softPackageReferences") {
                const soft = section.softPackageReferences;
                const items = Array.isArray(soft?.values) ? soft.values : [];
                if (items.length > 0) {
                    items.pop();
            ...
            const items = Array.isArray(section[field]) ? section[field] : [];
            if (items.length > 0) {
                items.pop();
                updatePageAfterTrim(section, field);
                return true;
            }
        }
        if (sections) {
            for (const name of ["dependencies", "assetRegistry", "exports", "imports", "names"]) {
                if (name in sections) {
...
function summary(path, requestedEngineVersion, options) {
    if (!requestedEngineVersion.startsWith("5.3"))
        throw failure("asset_version_incompatible", "This bundled Asset Worker is pinned to UE 5.3 package headers.");
    const resolved = headerPath(path);
    const stat = statSync(resolved.headerPath);
    if (stat.size < 32)
        throw failure("asset_corrupt", "Asset package is too small to contain a package summary.");
    ...
    const value = {
        ...
        counts: {
            ...
            dependencyEdges: Number(dependencies.dependsMap?.edges ?? 0),
        ...
        sectionDigests: {
            ...
            dependencies: {
                dependsMap: dependencies.dependsMap?.sha256 ?? null,
                softPackageReferences: dependencies.softPackageReferences?.normalizedSha256 ?? null,
            },
        },
        companion: resolved.companion,
    };
    const sections = {};
    if (options.sections.has("names"))
        sections.names = nameSection;
    if (options.sections.has("imports"))
        sections.imports = imports;
    if (options.sections.has("exports"))
        sections.exports = exports;
    if (options.sections.has("assetRegistry"))
        sections.assetRegistry = assetRegistry;
    if (options.sections.has("dependencies"))
...
process.stdin.on("data", (chunk) => { input += chunk; if (input.length > 1024 * 1024)
    process.exit(2); });
process.stdin.on("end", () => {
    try {
        ...
        const data = request.operation === "production.asset.package.diff"
            ? (() => {
                if (typeof request.otherAssetPath !== "string" || !existsSync(request.otherAssetPath))
                    throw failure("asset_diff_target_required", "otherAssetPath is required for package diff.");
