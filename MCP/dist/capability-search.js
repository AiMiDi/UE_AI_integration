export function compareCapabilityIds(left, right) {
...
function lowerAscii(text) {
    ...
    for (let index = 0; index < text.length; ++index) {
        ...
        result += String.fromCharCode(isAsciiUpper(code) ? code - 0x41 + 0x61 : code);
    }
    return result;
}
...
export function tokenizeCapabilitySearch(text) {
    const tokens = [];
    let current = "";
    const flush = () => {
    ...
    for (let index = 0; index < text.length; ++index) {
        ...
        const alphanumeric = isAsciiUpper(code) || isAsciiLower(code) || isAsciiDigit(code);
        if (!alphanumeric) {
            flush();
            continue;
        }
        ...
        current += String.fromCharCode(isAsciiUpper(code) ? code - 0x41 + 0x61 : code);
    }
    flush();
    return tokens;
}
function fieldScore(token, text, exactScore, substringScore) {
    if (text === undefined) {
        return 0;
    }
...
export function matchCapabilitySearch(query, capability) {
    ...
    for (const token of queryTokens) {
        let bestScore = 0;
        let bestField = "description";
        const consider = (candidateScore, field) => {
        ...
        consider(fieldScore(token, capability.search?.title, 4000, 3500), "title");
        ...
        consider(fieldScore(token, capability.description, 1000, 500), "description");
        if (bestScore === 0) {
            return undefined;
        }
        score += bestScore;
