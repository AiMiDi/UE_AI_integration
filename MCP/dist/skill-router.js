...
function includesOperation(skill, operation) {
    return skill.recipes.some((recipe) => recipe.steps.some((step) => step.operations.includes(operation)));
}
function matches(skill, args) {
    ...
    const searchable = [
        ...
        .join(" ")
        .toLocaleLowerCase();
    return ((args.domain === undefined || skill.domains.includes(args.domain)) &&
        (args.risk === undefined ||
...
function executionTool(operation) {
    const domain = operation.split(".", 1)[0];
    return `ue_${domain}`;
}
function recipeGuide(catalog, recipe) {
    const operations = [
    ...
    const guideStep = (step) => {
        if (step.route === "workflow") {
            return {
                ...
                note: "Build one deterministic asset-edit workflow, approve its exact plan digest, and execute the unchanged workflow. Do not issue these operations as unrelated short calls. Read the declared recipe reference for the exact scope and AST example.",
            };
        }
        return {
            id: step.id,
    ...
    return {
        contract: "Skill recipes are guidance, not executable workflows. Discover each exact schema before execution.",
...
function requireSkill(catalog, id) {
    if (!id) {
...
function requireRecipe(skill, id) {
    if (id === undefined) {
        return undefined;
    }
...
export function handleAgentSkills(catalog, args) {
    try {
        if (args.action === "list") {
            if (args.skill || args.recipe || args.reference) {
                throw new UEApiError({
                    code: "invalid_skill_request",
                    message: "list does not accept skill, recipe, or reference fields.",
                });
            }
            ...
            return formatJsonResponse({
                ...
                skills: page.map((skill) => catalog.summary(skill)),
                next: "Call ue_skills action=get for one package, then use ue_context for every operation before executing it.",
            });
        }
        ...
        if (args.action === "get") {
            if (args.query ||
                ...
                throw new UEApiError({
                    code: "invalid_skill_request",
                    message: "get accepts only skill and optional recipe fields.",
                });
            }
        ...
        if (args.action === "read") {
            if (!args.reference ||
                ...
                throw new UEApiError({
                    code: "invalid_skill_request",
                    message: "read requires skill and one declared reference, without filters.",
                });
            }
            const resource = skill.resources.find((candidate) => candidate.path === args.reference);
            if (!resource) {
                throw new UEApiError({
...
function matchesSkill(skill, args) {
