...
function boundedText(path: string): string {
  ...
  if (size > MAX_TEXT) throw new UEApiError({ code: "offline_file_too_large", message: `${basename(path)} exceeds the 4 MiB offline text limit.` });
  const bytes = readFileSync(path);
  const text = new TextDecoder("utf-8", { fatal: true }).decode(bytes);
  if (text.includes("\0")) throw new UEApiError({ code: "invalid_text_encoding", message: `${basename(path)} is not valid UTF-8 text.` });
  return text;
}

function resolveProjectFile(root: string, value: unknown, extensions: string[]): string {
  if (typeof value !== "string" || value.trim() === "") throw new UEApiError({ code: "offline_path_required", message: "A project-relative path is required." });
  const candidate = resolve(root, value);
  if (!inside(root, candidate)) throw new UEApiError({ code: "path_outside_allowed_root", message: "Offline paths must stay inside projectRoot." });
  if (!extensions.includes(extname(candidate).toLowerCase())) throw new UEApiError({ code: "offline_extension_forbidden", message: `Allowed extensions: ${extensions.join(", ")}.` });
  if (!existsSync(candidate) || !statSync(candidate).isFile()) throw new UEApiError({ code: "offline_file_not_found", message: "The requested project file does not exist." });
  const realCandidate = realpathSync(candidate);
  if (!inside(root, realCandidate)) throw new UEApiError({ code: "path_outside_allowed_root", message: "Offline paths must not escape projectRoot through a link or junction." });
  return realCandidate;
}

function uproject(root: string, requested?: unknown): { path: string; data: JsonObject } {
  ...
  if (candidates.length !== 1) throw new UEApiError({ code: "uproject_ambiguous", message: "projectRoot must contain exactly one .uproject or params.uproject must select one." });
  try {
    return { path: candidates[0]!, data: JSON.parse(boundedText(candidates[0]!)) as JsonObject };
  } catch (error) {
    if (error instanceof UEApiError) throw error;
    throw new UEApiError({ code: "uproject_invalid", message: `.uproject is invalid JSON: ${(error as Error).message}` });
  }
}
...
export class LocalAssetExecutor implements CapabilityExecutor {
    ...
    return new Promise((resolvePromise, rejectPromise) => {
      ...
      child.on("close", (code) => {
        ...
        try {
          ...
          rejectPromise(new UEApiError({ code: "asset_worker_invalid_response", message: errors || (error as Error).message }));
        }
      });
