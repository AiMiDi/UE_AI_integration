...
interface CliSpec {
  command: string;
  environmentKey: "UE_CLI" | "UE_WORKFLOW_CLI";
  developmentBuilds: string[];
...
function locateCli(
  ...
  const executableName =
    platform === "win32" ? `${spec.command}.exe` : spec.command;
  const configuredPath =
    environmentValue(env, spec.environmentKey)?.trim() || null;
  ...
  for (const buildDirectory of spec.developmentBuilds) {
    addCandidate(
      "development",
      join(pluginRoot, buildDirectory, "CLI", "Release", executableName),
    );
  }
...
export function locateShortCli(
  options: CliLocatorOptions = {},
): CliLocationResult {
  return locateCli(
    {
