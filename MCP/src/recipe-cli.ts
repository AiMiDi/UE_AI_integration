...
function parseRaw(arguments_: string[]): CliArguments {
  const result: CliArguments = { json: false };
  for (let index = 0; index < arguments_.length; index += 1) {
    const argument = arguments_[index]!;
    if (!argument.startsWith("--")) {
      if (result.command === undefined) result.command = argument;
      else throw new Error(`Unexpected positional argument ${argument}.`);
      continue;
    }
    const [name, inline] = argument.slice(2).split("=", 2);
    if (name === "json") { result.json = true; continue; }
    const value = inline ?? arguments_[++index];
    if (!value || value.startsWith("--")) throw new Error(`--${name} requires a value.`);
    if (name === "file") result.file = value;
...
function parseArguments(): CliArguments {
  ...
  if (argsFileIndex >= 0) {
    ...
    if (!path) throw new Error("--args-file requires a path.");
    const raw = JSON.parse(readFileSync(path, "utf8")) as unknown;
    if (!Array.isArray(raw) || !raw.every((item) => typeof item === "string")) throw new Error("Recipe CLI args file must contain a string array.");
    return parseRaw(raw);
  }
  return parseRaw(process.argv.slice(2));
}

function loadRecipe(path: string | undefined): unknown {
  if (!path) throw new UEApiError({ code: "recipe_file_required", message: "--file is required." });
...
export async function main(): Promise<number> {
  try {
    ...
    let data: unknown;
    if (args.command === "validate") data = runner.validate(loadRecipe(args.file));
    else if (args.command === "plan") data = runner.plan(loadRecipe(args.file));
    else if (args.command === "start") data = runner.start(loadRecipe(args.file), args.inputs ?? {}, args.approvePlan);
    else {
      if (!args.runId) throw new UEApiError({ code: "recipe_run_id_required", message: "--run-id is required." });
