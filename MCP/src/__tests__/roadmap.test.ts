...
test("admits only explicitly session-safe capabilities into Session Recipes", () => {
  ...
  assert.ok(unsafe.diagnostics.some(
    (diagnostic) => diagnostic.code === "recipe_session_capability_forbidden",
  ));
...
test("binds Session Recipe plans to materialized inputs, catalog, Editor, and stopped PIE", async () => {
  let pie: Record<string, unknown> = { state: "stopped", generation: 7 };
  const server = createServer(async (request, response) => {
    let text = "";
    for await (const chunk of request) text += String(chunk);
    const payload = text === "" ? {} : JSON.parse(text) as Record<string, unknown>;
  ...
  try {
    ...
    const binding = first.session as Record<string, unknown>;
    ...
    await assert.rejects(
      runner.planOnline(recipe, { functionName: "BeginFixture", objectRef: {} }),
      (error: unknown) => error instanceof UEApiError && error.code === "recipe_pie_must_be_stopped",
    );
  } finally {
    await new Promise<void>((resolvePromise) => server.close(() => resolvePromise()));
  }
...
test("fails a Session Recipe lease conflict before starting PIE", async () => {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (request.url === "/api/v1/clients/register") {
      ...
      if (request.url === "/api/execute" && payload.capability === "scene.pie.start") pieStartCalls += 1;
