...
test("locates ue-workflow-cli offline with deterministic precedence", () => {
  ...
  const executableName =
    process.platform === "win32" ? "ue-workflow-cli.exe" : "ue-workflow-cli";
...
test("locates the short-operation ue-cli via UE_CLI and packaged layout", () => {
  ...
  const executableName =
    process.platform === "win32" ? "ue-cli.exe" : "ue-cli";
  const configured = resolve("custom-tools", executableName);
  const packaged = join(fixturePluginRoot, "CLI", "bin", executableName);
...
test("pages and filters capability summaries without emitting schemas by default", async () => {
  ...
  const noNetworkClient: UEConnectionClient = {
    getHealth: async () => {
  ...
  const defaultResponse = await handleCapabilities(
    catalog,
    noNetworkClient,
    {},
  );
  ...
  assert.equal(
    defaultPayload.capabilities.some(
      (capability: Record<string, unknown>) =>
        Object.hasOwn(capability, "inputSchema"),
    ),
    false,
  );

  const filteredResponse = await handleCapabilities(
    catalog,
    noNetworkClient,
    {
      domain: "scene",
  ...
  assert.ok(
    filteredPayload.capabilities.every(
      (capability: { id: string; domain: string; kind: string }) =>
        capability.id.includes("pie.") &&
        capability.domain === "scene" &&
        capability.kind === "command",
    ),
  );

  const rankedResponse = await handleCapabilities(
    catalog,
    noNetworkClient,
    {
      domain: "blueprint",
  ...
  assert.equal(
    rankedPayload.capabilities[0].id,
  ...
  const phraseResponse = await handleCapabilities(
    catalog,
    noNetworkClient,
    {
      domain: "blueprint",
  ...
  assert.equal(
    phrasePayload.capabilities[0].id,
    "blueprint.comment.create_from_selection",
  );
  assert.deepEqual(
    phrasePayload.capabilities[0].match.matchedTokens,
    ["comment", "group"],
  );

  const exactResponse = await handleCapabilities(
    catalog,
    noNetworkClient,
    { operation: "scene.pie.start" },
  );
...
test("keeps ue_context directory-only by default and pages full schemas by domain", () => {
  ...
  assert.equal(
    directoryPayload.capabilityCount,
    catalog.summary().capabilityCount,
  );
  ...
  assert.ok(
    domainPayload.capabilities.every(
      (capability: Record<string, unknown>) =>
        Object.hasOwn(capability, "inputSchema"),
    ),
  );
...
test("forwards live capability filters and pagination directly to the editor", async () => {
  const catalog = loadCapabilityCatalog();
  let receivedQuery: UECapabilityQuery | undefined;
